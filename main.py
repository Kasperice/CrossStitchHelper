...
if __name__ == "__main__":
    ...
    if option.value == "single":
        ...
        if selected_color == "Custom":
            ...
            if selected_color > cross_stitch_helper.config.getint(
                "template", "number_of_colors"
            ):
                print(
                    f"Your image contains only {cross_stitch_helper.config.getint('template', 'number_of_colors')} "
                    f"colors."
                )
        new_scheme, threshold = cross_stitch_helper.prepare_scheme_for_color(
        ...
        while True:
            ...
            if "+" in option.value:
                ...
                new_scheme, threshold = cross_stitch_helper.prepare_scheme_for_color(
                    color=selected_color, custom_threshold=new_threshold
                )
            ...
            if option.value == "save":
                ...
                cv2.imwrite(
                    f"{page_filename}/res_{selected_color}_test.png", new_scheme
                )
                print(f"File saved to: {page_filename}/res_{selected_color}_test.png")
                if original_threshold != new_threshold:
                    print(
                        f"Overriding threshold value for color {selected_color} to {new_threshold}"
                    )
                    cross_stitch_helper.override_threshold_for_color(
                        color=selected_color, new_threshold=new_threshold
                    )
                break
